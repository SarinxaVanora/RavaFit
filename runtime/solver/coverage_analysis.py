...
def _source_hint_candidates(rows: list[dict[str, Any]], source_context: dict[str, Any] | None) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any] | None]:
    """Use cheap Penumbra option-name evidence before expensive body-geometry scoring."""
    ...
    context_tokens = option_tokens | group_tokens
    if not rows:
        return rows, {"used": False, "reason": "no candidates"}, None

    # Genital body variants are considered only when the selected option provides explicit evidence.
    ...
    # Exact normalized option -> variant is the cheapest/highest-value signal.
    normalized_option = " ".join(sorted(option_tokens))
    exact: list[dict[str, Any]] = []
    for row in rows:
        variant_tokens = _body_hint_tokens(row.get("variant"))
        if variant_tokens and " ".join(sorted(variant_tokens)) == normalized_option:
            exact.append(row)
    if len(exact) == 1:
        row = exact[0]
        return [row], {
            "used": True, "authoritative": True, "method": "exact-option-variant-name",
            "option": option_text, "candidate_count_before": len(rows), "candidate_count_after": 1,
        }, row
    ...
    for row in rows:
        ...
        # A candidate that misses a distinctive option token is not a useful narrowing hit.
    ...
    try:
        for hinted_slot, chosen in authoritative_hints.items():
            ...
            matches.append({
                ...
                "confidence": 1.0,
                "reason": f"Penumbra option name directly identifies {chosen.get('body')} / {chosen.get('variant')}",
            })
