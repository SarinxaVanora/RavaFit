...
def _dense_penetration_report(prod: Any, source: Any, positions: dict[str, np.ndarray], target_triangles: np.ndarray, margin: float) -> dict[str, Any]:
    ...
    rows=[];total_bad=0;worst=None;threshold=float(margin)-.00003
    for name in sorted(positions):
        data=source.data(name);faces=np.asarray(data.get("F",[]),dtype=np.int64);vertices=np.asarray(positions[name],dtype=np.float64)
        if not len(faces) or vertices.ndim!=2 or vertices.shape[1:]!=(3,):continue
        samples=np.einsum("bk,fkj->fbj",bary,vertices[faces]).reshape(-1,3)
        _,_,signed,_,_=occupancy(samples,target_triangles,k=48,exact_band=float(margin)+.0020);signed=np.asarray(signed,dtype=np.float64);bad=signed<threshold;count=int(np.count_nonzero(bad));minimum=float(np.min(signed)) if len(signed) else None
        if minimum is not None:worst=minimum if worst is None else min(worst,minimum)
        total_bad+=count;rows.append({"mesh":name,"faces":int(len(faces)),"samples":int(len(samples)),"penetrating_samples":count,"minimum_signed_mm":minimum*1000.0 if minimum is not None else None})
    return {"enabled":True,"margin_mm":float(margin*1000.0),"penetrating_samples":int(total_bad),"minimum_signed_mm":worst*1000.0 if worst is not None else None,"meshes":rows,"policy":"dense face-interior samples must remain outside the complete selected target-body collision surface"}
...
def install_dense_final_target_occupancy(prod: Any) -> None:
    ...
    def guarded(source,cache,positions,skinning,records,contexts,local_affine_quality_rms_mm):
        candidate,skinning_out,records_out,stats=original(source,cache,positions,skinning,records,contexts,local_affine_quality_rms_mm)
        ...
        corrected,clearance_report=clear(source,candidate,target_collision,target_support,margin_m=.00035,maximum_vertex_move_m=.00800,maximum_step_m=.00125,max_passes=10)
        before={name:np.asarray(candidate[name],dtype=np.float64) for name in candidate};candidate={name:np.asarray(value,dtype=np.float64) for name,value in corrected.items()};changed=[name for name in candidate if name in before and np.any(np.linalg.norm(candidate[name]-before[name],axis=1)>1e-8)]
        for name in changed:records_out.setdefault(name,{})["dense_final_target_occupancy_applied"]=True
        ...
        merged_stats=dict(stats or {});merged_stats["dense_final_target_occupancy"]={"changed_meshes":changed,"changed_mesh_count":int(len(changed)),"clearance":clearance_report,"validation":validation};return candidate,skinning_out,records_out,merged_stats
