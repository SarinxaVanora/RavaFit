...
def graft_native_body(imported_mdl: str | Path, output_mdl: str | Path, conversion_report: str | Path, target_body_mdls: dict[str, str], source_model_mdl: str | Path | None = None) -> dict[str, Any]:
    ...
    # Native UBYTE joint indices are local to the mesh bone set, so restore the exact name order.
    ...
    for bone_set_index, native_names in bone_set_requirements.items():
        missing = [name for name in native_names if name not in output_global]
        if missing:
            raise ValueError(f'Reconstructed model lacks native body bones required by bone set {bone_set_index}: {missing[:20]}')
        translated = [output_global[name] for name in native_names]
    ...
    for om, row in mapping_by_output.items():
        ...
        bones_exact = _bone_names(final, om) == _bone_names(native, nm)
        ...
        if not all(item['exact'] for item in stream_checks) or not index_exact or not bones_exact or not declaration_exact or not attribute_masks_exact:
        ...
        body_checks[str(om)] = {
            ...
            'indices_exact': index_exact, 'bone_names_exact': bones_exact, 'declaration_exact': declaration_exact,
